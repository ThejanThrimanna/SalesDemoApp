package com.apps.tdt.thejan.salesdemoapp.services;
...
public class ServiceGenerator {
    ...
    public static <S> S CreateServiceWithTimeout(Class<S> serviceClass, String serviceUrl) {
        ...
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl("http://" + serviceUrl)
                .client(okHttpClient.newBuilder().connectTimeout(2, TimeUnit.MINUTES).readTimeout(30, TimeUnit.SECONDS).writeTimeout(30, TimeUnit.SECONDS).build())
                .addConverterFactory(GsonConverterFactory.create(gson))
                .build();
